...
if(navToggle){navToggle.addEventListener('click',()=>{const open=siteNav.classList.toggle('open'); navToggle.setAttribute('aria-expanded',open?'true':'false');});}

// Year
const y=document.getElementById('year'); if(y) y.textContent=new Date().getFullYear();

// 8 produkter (bruges i grid). Billeder = placeholders, kan skiftes til dine egne URLs senere.
const products=[
  ...
  {id:'bowl-ceramic',   title:'Keramisk vandskål',        price:179, category:'skåle',     img:'https://placehold.co/800x600?text=Skål',       meta:'Skridsikker bund'},
...
if(filterBtns.length){
  filterBtns.forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      cards.forEach(c=>{
        const vis = filter==='alle' || c.dataset.category===filter;
